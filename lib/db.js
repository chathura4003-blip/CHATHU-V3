...
// Allow operators to redirect the JSON store via env var. Useful for Docker
// volumes that mount a directory (named volumes can't bind-mount a single
// file). When DB_FILE is set we ensure the parent directory exists so a
// fresh container can boot without a manual mkdir.
const DB_PATH = (() => {
  const override = process.env.DB_FILE && process.env.DB_FILE.trim();
  const resolved = override
    ? (path.isAbsolute(override) ? override : path.join(__dirname, "..", override))
    : path.join(__dirname, "..", "db.json");
  try { fs.mkdirSync(path.dirname(resolved), { recursive: true }); } catch {}
  return resolved;
})();
