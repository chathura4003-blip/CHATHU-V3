  ...
  try {
    ...
    $(site.sel)
      ...
      .each((_, el) => {
        ...
        // Skip placeholder / disabled anchors — blindly concatenating
        // "javascript:void(0)" onto site.base produces URLs like
        // "https://example.comjavascript:void(0)" which crash downstream
        // (yt-dlp, URL parser) with "nonnumeric port: 'void(0)'".
        const looksLikeLink =
          typeof href === "string" &&
          href.length > 0 &&
          !/^\s*(?:javascript|mailto|tel|data|about):/i.test(href) &&
          !href.startsWith("#");
        if (title && looksLikeLink) {
          let url;
