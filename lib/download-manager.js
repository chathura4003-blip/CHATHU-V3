...
function isLikelyVideoUrl(candidate) {
  if (typeof candidate !== "string") return false;
  const trimmed = candidate.trim();
  if (!trimmed) return false;
  // Placeholder / disabled anchors that sometimes leak through scrapers.
  if (/^(?:javascript|mailto|tel|data|about):/i.test(trimmed)) return false;
  try {
...
async function getMetadata(videoUrl) {
  if (!isLikelyVideoUrl(videoUrl)) return null;
  try {
