...
function _ensurePython3() {
  ...
  try {
    ...
    return true; // python is available (symlink existed or was just created)
  } catch {}
  return false;
}
...
function _downloadStandalone(downloadPath) {
  ...
  // Download to a temp file first so a partial/aborted download never
  // corrupts an existing binary. Atomically swap on success.
  const tmpPath = `${downloadPath}.${process.pid}.tmp`;
  try {
    execSync(
      `curl -fL "${url}" -o "${tmpPath}" || wget -q "${url}" -O "${tmpPath}"`,
      { stdio: "pipe", timeout: 180000, shell: "/bin/bash" }
    );
    if (!fs.existsSync(tmpPath) || fs.statSync(tmpPath).size < 1024) {
      throw new Error("download produced empty/missing file");
    }
    if (!isWin) execSync(`chmod +x "${tmpPath}"`, { stdio: "pipe" });
    try {
      fs.renameSync(tmpPath, downloadPath);
    } catch (renameErr) {
      // Cross-device rename failure — fall back to copy + unlink.
      fs.copyFileSync(tmpPath, downloadPath);
